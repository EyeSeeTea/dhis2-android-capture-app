...
This project uses Material 3 with the DHIS2 design system. Before designing:
- Check `ui-components/` for existing shared components and design tokens
- Review `app/src/main/res/values/colors.xml` and `themes.xml` for the color system
- Check `gradle/libs.versions.toml` for the `designSystem` version being used
- Use Material 3 components and patterns (top app bars, FABs, bottom sheets, cards)
- Support light and dark themes
- Use `get_variables` / `set_variables` to align Pencil variables with the
project's Material 3 tokens (colorScheme, typography scale, shape scale).
