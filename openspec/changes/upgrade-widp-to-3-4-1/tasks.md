...
- [x] 4.5b Notifications — **DI re-anchored to Koin (fixes a runtime NPE found in manual validation)**. Task 4.5 re-anchored the call path but assumed the Dagger graph survived; it did not. `MainActivity` no longer runs `inject()`, so the inherited `@Inject notificationsPresenter` stayed `null`. Moved the whole graph to Koin (`NotificationsKoinModule.kt`, new file), deleted the Dagger `NotificationsModule.kt`/`NotificationsComponent.kt`, and `ActivityGlobalAbstract` now resolves the presenter via `getNotificationsPresenter()`. Oslo files carrying notifications wiring: 4 → 2. `MainActivity` unchanged. New `NotificationsModuleTest` guards the graph (verified by mutation). See the notes file
- [x] 4.5c Notifications — **two display defects fixed, found in manual validation on device**. (1) nothing refreshed once the asynchronous download landed, so the result only surfaced if the user passed through Home; (2) `refresh()` consumed `ShowNotifications.isPending` even with an empty list, which made the failure systematic for single-program users because the program screen's `onCreate` burned the flag while the download was still in flight. Both fixed in `NotificationsPresenter.kt` (level 2, ours) plus one argument at the `MainActivity` call site. `NotificationsPresenterTest` added (6 tests, both fixes verified by mutation). No spec change: the branch behaviour violated the existing "displayed on activity resume" requirement, so this restores conformance. See the notes file
