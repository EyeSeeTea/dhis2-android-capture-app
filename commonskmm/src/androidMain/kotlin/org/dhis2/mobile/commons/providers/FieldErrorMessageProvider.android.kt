...
import org.dhis2.mobile.commons.resources.value_not_unique
...
actual class FieldErrorMessageProvider {
    ...
    actual suspend fun getFriendlyErrorMessage(error: Throwable) =
        getString(parseErrorToMessageResource(error))
    ...
    private fun parseErrorToMessageResource(error: Throwable) = when (error) {
        ...
        is ValueNotUniqueFailure ->  Res.string.value_not_unique
        else -> Res.string.invalid_field
    }

    private fun getTrueOnlyError(error: TrueOnlyFailure) = when (error) {
    ...
    private fun getTextError(error: TextFailure) = when (error) {
        TextFailure.TooLargeTextException -> Res.string.error_text_too_long
    }

    private fun getLetterError(error: LetterFailure) = when (error) {
    ...
    private fun getTimeError(error: TimeFailure) = when (error) {
        TimeFailure.ParseException -> Res.string.error_time_parsing
    }

    private fun getDateTimeError(error: DateTimeFailure) = when (error) {
        DateTimeFailure.ParseException -> Res.string.error_date_time_parsing
    }

    private fun getDateError(error: DateFailure) = when (error) {
        DateFailure.ParseException -> Res.string.error_date_parsing
    }

    private fun getBooleanError(error: BooleanFailure) = when (error) {
    ...
    suspend fun mandatoryWarning(): String {
        return getString(Res.string.required)
    }

    suspend fun defaultValidationErrorMessage(): String {
        return getString(Res.string.validation_error_message)
    }
}
