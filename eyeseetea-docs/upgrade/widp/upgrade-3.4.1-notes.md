...
| `gradle.properties` | accept_theirs → **corrected to manual_reapply_on_theirs** | build tuning | fork build tuning | resolved_manual_merge | **The original note here was wrong.** It claimed "WIDP had no customization here vs merge-base"; the merge-base (3.3.1) has `-Xmx4096M` and `develop-widp` has `-Xmx8g -XX:MaxMetaspaceSize=1g`, so it was a fork customization and `accept_theirs` silently dropped it. Found on 2026-08-21 by diffing against `develop-eyeseetea`, which kept its own. Restored to match the baseline |
