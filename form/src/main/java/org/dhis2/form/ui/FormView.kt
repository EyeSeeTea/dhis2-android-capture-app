...
class FormView : Fragment() {
    ...
    override fun onCreateView(
        ...
        return ComposeView(requireContext()).apply {
            ...
            setContent {
                ...
                // Base behavior: map the form with the raw field list from the repository.
                val finalItems = onFieldsLoadingListener?.invoke(items ?: emptyList()) ?: items
                onFieldsLoadedListener?.invoke(finalItems ?: emptyList())
    ...
    private fun showOrgUnitDialog(uiEvent: RecyclerViewUiEvents.OpenOrgUnitDialog) {
        OUTreeFragment
            ...
            // EyeSeeTea customization - Validate or hide orgunit by Teamprofile
            // Base behavior: open org unit dialog using only the selector scope.
            .withTeamValidationData(
                uiEvent.orgUnitValidationData?.programOrDataSetUid,
                uiEvent.orgUnitValidationData?.period,
            )
            .build()
