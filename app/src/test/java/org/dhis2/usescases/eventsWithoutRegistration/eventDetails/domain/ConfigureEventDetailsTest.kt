...
class ConfigureEventDetailsTest {
    ...
    @Test
    fun `action button should be visible on new event`() =
        runBlocking {
    ...
    @Test
    fun `action button should not be visible on new event`() =
        runBlocking {
            // Given user creates a new event
            configureEventDetails =
                ConfigureEventDetails(
    ...
    @Test
    fun `action button should be visible on existing event`() =
        runBlocking {
    ...
    @Test
    fun `action button should hide button on a non editable existing event`() =
        runBlocking {
            // Given user is in an existing event
            whenever(repository.getEvent()) doReturn event
