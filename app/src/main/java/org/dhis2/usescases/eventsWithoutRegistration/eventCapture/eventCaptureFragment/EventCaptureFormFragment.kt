...
class EventCaptureFormFragment :
    ...
    override fun selectUPG(orgUnitUid: String) {
        // EyeSeeTea customization - Select UPG
        // Base behavior: focus on the field and edit its raw value directly in the form.
        val dialog = SelectUPGDialog.newInstance(orgUnitUid)

        dialog.setUPGSelectedListener { upg: UPGItem? ->
            presenter.onUPGSelected(upg!!)
            Unit
        }

        dialog.show(activity.supportFragmentManager, "SELECT_UPG_TAG")
    }
