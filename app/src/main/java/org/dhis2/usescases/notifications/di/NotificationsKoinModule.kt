...
/**
 * Koin graph for the notifications capability. Replaced the Dagger `NotificationsModule` in
 * 3.4.1: upstream migrated `MainActivity` to Koin and dropped its `inject()` call, which was the
 * only thing populating `ActivityGlobalAbstract.notificationsPresenter`.
 *
 * `BasicPreferenceProvider` is built inline instead of being published to the graph: it is an
 * Oslo type we do not own, and declaring it here would collide the day upstream registers it in
 * Koin.
 */
