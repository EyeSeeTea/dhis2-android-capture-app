...
open class App : Application(), Components, DefaultLifecycleObserver {
    ...
    private var changeServerURLComponent: ChangeServerURLComponent? = null
    ...
    // EyeSeeTea customization - Change Server URL
    fun createChangeServerULComponent(
        changeServerURLModule: ChangeServerURLModule,
    ): ChangeServerURLComponent {
        changeServerURLComponent = userComponent?.plus(changeServerURLModule)
        return changeServerURLComponent!!
    }

    // EyeSeeTea customization - Change Server URL
    // Replaces the call to releaseSessionComponent() the dialog used to make: upstream 3.4.1
    // removed SessionComponent, and this dialog should release its own subcomponent anyway.
    fun releaseChangeServerURLComponent() {
        changeServerURLComponent = null
    }
